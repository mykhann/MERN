...
const Jobs = () => {
  ...
  useEffect(()=>{
    if (searchJobByText){
     const filteredjob= adminjobs.length>0 &&adminjobs.map((job)=>{
        if (!searchJobByText) {return null}
                ...
                return (
                  <tr className="border-b border-gray-200">
                    ...
                    <td className="py-2 px-4 text-left">
                      ...
                      <Popover>
                        <PopoverTrigger>
                          <button  onClick={()=>navigate(`/admin/companies/${company?._id}`)}className="text-red-500 hover:translate-x-1 transition-shadow transform-gpu hover:text-red-700">
                            <Pen />
                          </button>
                        </PopoverTrigger>
